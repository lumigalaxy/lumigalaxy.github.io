...
const SCHEMA = {
  ...
  // Pure data prefs
  lang: { type: "string", default: "en", enum: ["en", "zh", "es"] },
  ...
  waterReminder: {
    ...
    normalize: normalizeWaterReminder,
  },
  // Translator: ⌘⇧T (Cmd+Shift+T) opens a small popup near the cursor.
  // Backend is the unofficial Youdao dictionary endpoint.
  translator: {
    type: "object",
    defaultFactory: () => ({
      enabled: true,
...
function normalizeWaterReminder(value, defaultsValue) {
  if (!isPlainObject(value)) return defaultsValue;
  const out = { ...defaultsValue };
  if (typeof value.enabled === "boolean") out.enabled = value.enabled;
  if (typeof value.intervalMinutes === "number" && Number.isFinite(value.intervalMinutes)
      && value.intervalMinutes >= 1 && value.intervalMinutes <= 480) {
    out.intervalMinutes = Math.round(value.intervalMinutes);
  }
  if (typeof value.snoozeMinutes === "number" && Number.isFinite(value.snoozeMinutes)
      && value.snoozeMinutes >= 1 && value.snoozeMinutes <= 60) {
    out.snoozeMinutes = Math.round(value.snoozeMinutes);
  }
  if (_isHHMM(value.quietStart)) out.quietStart = value.quietStart;
  if (_isHHMM(value.quietEnd))   out.quietEnd   = value.quietEnd;
  return out;
}

function normalizeTranslator(value, defaultsValue) {
  if (!isPlainObject(value)) return defaultsValue;
  const out = { ...defaultsValue };
  if (typeof value.enabled === "boolean") out.enabled = value.enabled;
  if (typeof value.shortcut === "string" && value.shortcut.trim()) {
    out.shortcut = value.shortcut.trim();
  }
  return out;
}
