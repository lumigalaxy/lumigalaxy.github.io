...
const chatWindowModule = require("./chat-window");
const chatBubbleModule = require("./chat-bubble");
const waterReminderModule = require("./water-reminder");
...
const _settingsController = createSettingsController({
  ...
  injectedDeps: {
    ...
    triggerWaterReminder: () => waterReminderModule.triggerNow(),
...
function repositionFloatingBubbles() {
  ...
  repositionUpdateBubble();
  if (chatBubbleModule && chatBubbleModule.isBubbleOpen && chatBubbleModule.isBubbleOpen()) {
    chatBubbleModule.repositionBubbles();
  }
  if (waterReminderModule && waterReminderModule.isBubbleOpen && waterReminderModule.isBubbleOpen()) {
    waterReminderModule.reposition();
  }
}
...
const _menuCtx = {
  ...
  openChatWindow: () => chatWindowModule.openChatWindow(),
  toggleChatBubble: () => chatBubbleModule.toggleChatBubble(),
  closeChatBubble: () => chatBubbleModule.closeChatBubble(),
  openTranslator: () => translatorModule.openTranslator(),
  translateClipboard: () => translatorModule.translateClipboard(),
  triggerWaterReminder: () => waterReminderModule.triggerNow(),
};
chatBubbleModule.init(_menuCtx);
waterReminderModule.init(_menuCtx);
translatorModule.init(_menuCtx);
...
function wireSettingsSubscribers() {
  _settingsController.subscribe(({ changes }) => {
    ...
    if ("waterReminder" in changes) {
      try { waterReminderModule.updateConfig(); } catch (err) {
        console.warn("Clawd: waterReminder.updateConfig failed:", err && err.message);
      }
    }
    if ("translator" in changes) {
      try { translatorModule.applyShortcut(); } catch (err) {
        console.warn("Clawd: translator.applyShortcut failed:", err && err.message);
      }
    }
    if ("theme" in changes) {
      // Water reminder is alien-only; flipping theme should clear any lingering bubble.
      try { waterReminderModule.updateConfig(); } catch {}
    }
...
const REMOVE_THEME_DIALOG_STRINGS = {
  ...
  es: {
    ...
    detail: "Esta acción no se puede deshacer. Todos los archivos del tema se eliminarán del disco.",
  },
};
...
const CLAUDE_HOOKS_DIALOG_STRINGS = {
  ...
  es: {
    disableTitle: "¿Desactivar la gestión automática de hooks de Claude?",
    disableDetail: "Si no los eliminas ahora, los hooks de Claude instalados en `~/.claude/settings.json` se mantendrán.",
    disableOnly: "Solo desactivar la gestión automática",
...
function createWindow() {
  ...
  ipcMain.on("open-chat-window", () => chatWindowModule.openChatWindow());
...
if (!gotTheLock) {
  ...
  app.on("before-quit", () => {
    ...
    try { waterReminderModule.shutdown(); } catch {}
    try { translatorModule.shutdown(); } catch {}
    stopTopmostWatchdog();
