...
function delegateWindowToStationarySpace(nsWindow) {
  try {
    ...
    if (!warnedSkyLightFailure) {
      console.warn("Alien: failed to move macOS window into stationary SkyLight space:", err.message);
...
function applyStationaryCollectionBehavior(browserWindow) {
  ...
  try {
    ...
    if (!warnedApplyFailure) {
      console.warn("Alien: failed to apply macOS stationary window behavior:", err.message);
