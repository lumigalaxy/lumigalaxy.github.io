...
module.exports = function initMenu(ctx) {
  ...
  function createTray() {
    ...
    ctx.tray.setToolTip("Alien Desktop Pet");
    buildTrayMenu();
  }
  ...
  function buildTrayMenu() {
    ...
    items.push(
      { type: "separator" },
      {
        label: t("chatWithAlien"),
        click: () => ctx.openChatWindow && ctx.openChatWindow(),
      },
      {
        label: t("translateClipboard"),
        click: () => ctx.translateClipboard && ctx.translateClipboard(),
      },
      {
        label: t("settings"),
