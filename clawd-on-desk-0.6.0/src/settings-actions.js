...
const updateRegistry = {
  ...
  lang: requireEnum("lang", ["en", "zh", "es"]),
  ...
  showTray(value, { snapshot }) {
    ...
    if (!value && snapshot && snapshot.showDock === false) {
      return {
        status: "error",
        message: "Cannot hide Menu Bar while Dock is also hidden — Alien would become unquittable.",
      };
    }
    return { status: "ok" };
  },
  showDock(value, { snapshot }) {
    ...
    if (!value && snapshot && snapshot.showTray === false) {
      return {
        status: "error",
        message: "Cannot hide Dock while Menu Bar is also hidden — Alien would become unquittable.",
      };
    }
    return { status: "ok" };
  },
  ...
  // Water reminder (Pomodoro-style hydration prompt) + translator popup.
  // The full object is always sent from the settings panel; the schema's
  // normalize() fills in any missing fields on save.
  waterReminder: requirePlainObject("waterReminder"),
  translator: requirePlainObject("translator"),
...
function triggerWaterReminder(_payload, deps) {
  if (!deps || typeof deps.triggerWaterReminder !== "function") {
    return { status: "error", message: "triggerWaterReminder requires deps.triggerWaterReminder" };
  }
  try {
    deps.triggerWaterReminder();
    return { status: "ok" };
  } catch (err) {
    return { status: "error", message: `triggerWaterReminder: ${err && err.message}` };
  }
}
