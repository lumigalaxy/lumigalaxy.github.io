...
// Auto-start OpenClaw gateway on SessionStart
let gatewayStarted = false;
function ensureOpenClawGateway() {
  if (gatewayStarted) return;
  gatewayStarted = true;

  try {
    // Check if gateway is already running by testing port 18789
    ...
    client.on("error", () => {
      ...
      try {
        // Find openclaw path
        let openclawPath = "openclaw";
        const npmGlobalBin = path.join(process.env.HOME, ".npm-global", "bin", "openclaw");
        if (fs.existsSync(npmGlobalBin)) {
          openclawPath = npmGlobalBin;
        }

        // Start gateway in background, detached
        const child = spawn(openclawPath, ["gateway"], {
          detached: true,
          stdio: "ignore",
