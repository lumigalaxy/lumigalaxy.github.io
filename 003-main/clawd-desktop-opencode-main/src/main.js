...
// Temporary crash logging
process.on("uncaughtException", (e) => {
  fs.writeFileSync("/tmp/clawd-crash.log", e.stack + "\n", { flag: "a" });
});
process.on("unhandledRejection", (e) => {
  fs.writeFileSync("/tmp/clawd-crash.log", String(e) + "\n", { flag: "a" });
});

const { GrowthEngine } = require("./growth.js");
const { getMoodBehavior } = require("./mood.js");
const { EasterEggEngine } = require("./easter-eggs.js");
const { getCatalog, ACCESSORIES } = require("./wardrobe.js");

const isMac = process.platform === "darwin";

// ── Event name normalization (OpenCode → Claude Code equivalents) ──
const EVENT_NORMALIZATION = {
  // OpenCode events → Claude Code compatible event names
  "session.created": "SessionStart",
  "session.deleted": "SessionEnd",
  "tui.prompt.append": "UserPromptSubmit",
  "tool.execute.before": "PreToolUse",
  "tool.execute.after": "PostToolUse",
  "tool.execute.failure": "PostToolUseFailure",
  "session.idle": "Stop",
  ...
  "message.part.updated": "Elicitation",
  "file.watcher.updated": "WorktreeCreate",
};
...
function savePrefs() {
  ...
  const data = {
    ...
    showTray, showDock, introSeen,
  };
  try { fs.writeFileSync(PREFS_PATH, JSON.stringify(data)); } catch {}
}

// ── Growth Engine & Easter Eggs ──
let growth = null; // initialized in app.whenReady
let easterEggs = null;

// ── Game windows (module-scoped so mainTick can pause hit-testing) ──
let danceGameWin = null;
let bugGameWin = null;
let inlineGameActive = false;
...
const STATE_SVGS = {
  ...
  dancing: ["clawd-dance-disco.svg", "clawd-dance-robot.svg", "clawd-dance-spin.svg"],
};
...
const STATE_PRIORITY = {
  error: 8, notification: 7, sweeping: 6, attention: 5, dancing: 5,
...
function startMainTick() {
  ...
  mainTickTimer = setInterval(() => {
    ...
    // ── Hit-test (always-on) — paused while game windows are open ──
    const bounds = win.getBounds();
    if (inlineGameActive || introActive) {
      // Inline game/intro needs mouse — skip hit-test, keep events enabled
    } else if (!dragLocked) {
...
// ── Session management ──
const ONESHOT_STATES = new Set(["attention", "error", "sweeping", "notification", "carrying", "dancing"]);

function updateSession(sessionId, state, event, sourcePid, cwd) {
  // Feed events to growth engine for XP tracking
  if (growth) {
    const concurrent = [...sessions.values()].filter(s => s.state === "juggling").length;
    growth.onEvent(event || state, { concurrent });
  }

  // Check easter eggs
  if (easterEggs && growth) {
    const eggCtx = {
      event: event || state,
      state,
      recentErrors: growth.data.stats.totalErrors,
      consecutiveErrors: growth.data.stats.totalErrors,
      sessionCompleted: growth.data.stats.sessionCompleted,
      streak: growth.data.streak.current,
      level: growth.data.growth.level,
    };
    const egg = easterEggs.check(eggCtx);
    if (egg) {
      if (egg.speech) sendToRenderer("easter-egg-speech", egg.speech, "normal");
      else if (egg.text) sendToRenderer("easter-egg-speech", egg.text, egg.mode || "normal");
      if (egg.state) setState(egg.state);
    }
  }
    ...
    if (req.method === "POST" && req.url === "/state") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          // Normalize event name (OpenCode → Claude Code)
          const rawEvent = event || "";
          event = EVENT_NORMALIZATION[rawEvent] || rawEvent;
...
function createWindow() {
  ...
  if (prefs && prefs.introSeen) introSeen = true;
  ...
  if (!introSeen) {
    ...
    win.webContents.once("did-finish-load", () => {
      // Enable mouse events so user can click the ship
      win.setIgnoreMouseEvents(false);
      mouseOverPet = true;
      sendToRenderer("play-intro");
    });
  }
  ...
  ipcMain.on("request-dance", () => {
    if (!miniMode && !doNotDisturb && currentState !== "dancing") setState("dancing");
  });

  // ── Growth system IPC ──
  ipcMain.on("poke-clawd", () => {
    if (growth) growth.onEvent("poke");
    if (easterEggs) {
      easterEggs.trackPoke();
      const egg = easterEggs.check({ event: "poke", state: currentState, recentErrors: 0, streak: 0, level: 1, sessionCompleted: 0 });
      if (egg) {
        if (egg.text) sendToRenderer("easter-egg-speech", egg.text, egg.mode || "normal");
        if (egg.state) setState(egg.state);
      }
    }
  });
  ipcMain.handle("get-growth-summary", () => {
    if (!growth) return null;
    return growth.getSummary(lang);
  });
  ipcMain.handle("get-dialogue-line", () => {
    const { selectLine } = require("./dialogue.js");
    const now = new Date();
    const s = growth ? growth.data : null;
    const ctx = {
      state: currentState,
      hour: now.getHours(),
      dayOfWeek: now.getDay(),
      mood: s ? s.mood.value : 50,
      streak: s ? s.streak.current : 0,
      level: s ? s.growth.level : 1,
      recentErrors: s ? s.stats.totalErrors : 0,
      lastEvent: currentState,
      idleMinutes: currentState === "idle" ? Math.floor((Date.now() - stateChangedAt) / 60000) : 0,
      levelUpRecent: false,
    };
    return selectLine(ctx);
  });
  ipcMain.handle("get-dialogue-context", () => {
    const now = new Date();
    const s = growth ? growth.data : null;
    return {
      state: currentState,
  ...
  // ── Wardrobe IPC ──
  ipcMain.handle("get-wardrobe", () => {
    if (!growth) return { catalog: [], equipped: {} };
    return {
      catalog: getCatalog(growth.data, lang),
      equipped: growth.data.equipped || {},
    };
  });
  ipcMain.on("equip-item", (_, itemId) => {
    if (!growth || !growth.data) return;
    const acc = ACCESSORIES[itemId];
    if (!acc) return;
    // Check unlock
    try { if (!acc.unlock(growth.data)) return; } catch { return; }
    // Equip by category (one per category)
    if (!growth.data.equipped) growth.data.equipped = {};
    growth.data.equipped[acc.category] = itemId;
    growth.save();
    sendToRenderer("wardrobe-update", growth.data.equipped);
  });
  ipcMain.on("unequip-item", (_, category) => {
    if (!growth || !growth.data) return;
    if (!growth.data.equipped) return;
    growth.data.equipped[category] = null;
    growth.save();
    sendToRenderer("wardrobe-update", growth.data.equipped);
  });
  ...
  // ── Inline Games (no separate windows — games play on the pet itself) ──
  const DANCE_SVGS = ["clawd-dance-disco.svg", "clawd-dance-robot.svg", "clawd-dance-spin.svg"];
  let danceHitIdx = 0;

  function startInlineGame(gameType) {
    if (inlineGameActive || miniMode || doNotDisturb) return;
    inlineGameActive = true;
    // Disable mouse-ignore so the window can receive keyboard input
    win.setIgnoreMouseEvents(false);
    mouseOverPet = true;
    win.focus();
    win.webContents.focus();
    if (gameType === "dance") setState("dancing");
    sendToRenderer("start-game", gameType);
  }

  ipcMain.on("game-result", (_, gameType, result) => {
    inlineGameActive = false;
    // Restore normal mouse handling
    win.setIgnoreMouseEvents(true);
    mouseOverPet = false;
    if (growth && result && result.grade) {
      growth.onEvent("dance_game_result", { grade: result.grade });
    }
    // Dance animation on game hit
    if (gameType === "dance-hit") {
      const svg = DANCE_SVGS[danceHitIdx % DANCE_SVGS.length];
      danceHitIdx++;
      if (currentState !== "dancing" || currentSvg !== svg) applyState("dancing", svg);
      if (autoReturnTimer) { clearTimeout(autoReturnTimer); autoReturnTimer = null; }
      autoReturnTimer = setTimeout(() => { autoReturnTimer = null; applyState("idle"); }, 3000);
      inlineGameActive = true; // still playing
      win.setIgnoreMouseEvents(false);
      mouseOverPet = true;
      return;
    }
    // Game ended
    setTimeout(() => {
      if (currentState === "dancing" || currentState === "working") applyState("idle");
    }, 2000);
  });

  global._openDanceGame = () => startInlineGame("dance");
  global._openBugGame = () => startInlineGame("bug");
...
function buildContextMenu() {
  const template = [
    ...
    {
      label: t("dance"),
      enabled: !miniMode && !doNotDisturb && currentState !== "dancing",
      click: () => setState("dancing"),
    },
    {
      label: t("danceGame"),
      enabled: !miniMode && !doNotDisturb,
      click: () => { if (global._openDanceGame) global._openDanceGame(); },
    },
    {
      label: t("bugGame"),
      enabled: !miniMode && !doNotDisturb,
      click: () => { if (global._openBugGame) global._openBugGame(); },
    },
    { type: "separator" },
    (() => {
      const s = growth ? growth.getSummary(lang) : null;
      const lvLabel = s ? `Lv.${s.level} ${s.title}` : "Lv.1";
      const xpLabel = s ? `XP: ${s.xp}${s.nextXp ? "/" + s.nextXp : ""}` : "";
      const moodLabel = s ? `${t("status")}: ${s.moodLabel}` : "";
      const streakLabel = s ? (lang === "zh" ? `连续 ${s.streak} 天` : `${s.streak}-day streak`) : "";
      return {
        label: `◈ ${lvLabel}`,
        submenu: [
          { label: xpLabel, enabled: false },
          { label: moodLabel, enabled: false },
          { label: streakLabel, enabled: false },
          { type: "separator" },
          { label: lang === "zh" ? `成就: ${s ? s.achievements : 0} 个` : `Achievements: ${s ? s.achievements : 0}`, enabled: false },
        ],
      };
    })(),
    (() => {
      const catalog = growth ? getCatalog(growth.data, lang) : [];
      const equipped = growth && growth.data.equipped ? growth.data.equipped : {};
      const sub = [];
      const categories = ["hat", "scarf", "effect"];
      for (const cat of categories) {
        ...
        for (const item of items) {
          ...
          const label = item.unlocked
            ? `${item.emoji} ${item.name}${isEquipped ? " ✓" : ""}`
            : `🔒 ${item.name} (${item.unlockHint})`;
          sub.push({
            label,
            enabled: item.unlocked,
            click: () => {
              if (isEquipped) {
                sendToRenderer("wardrobe-update", { ...equipped, [cat]: null });
                if (growth) { growth.data.equipped[cat] = null; growth.save(); }
              } else {
                const newEquipped = { ...equipped, [cat]: item.id };
                sendToRenderer("wardrobe-update", newEquipped);
                if (growth) { growth.data.equipped[cat] = item.id; growth.save(); }
              }
            },
          });
        }
      ...
      if (sub.length > 0 && sub[sub.length - 1].type === "separator") sub.pop();
      return {
        label: `🎨 ${t("wardrobe")}`,
        submenu: sub.length > 0 ? sub : [{ label: lang === "zh" ? "暂无装扮" : "No items yet", enabled: false }],
      };
    })(),
    { type: "separator" },
    {
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    // Initialize growth engine & easter eggs
    growth = new GrowthEngine(app.getPath("userData")).load();
    easterEggs = new EasterEggEngine();
    growth.on("levelup", (data) => {
      sendToRenderer("growth-levelup", data);
      // Check level-based easter eggs
      if (easterEggs) {
        const egg = easterEggs.check({ event: "levelup", level: data.level, state: currentState, streak: growth.data.streak.current, recentErrors: 0, sessionCompleted: growth.data.stats.sessionCompleted });
        if (egg && egg.text) sendToRenderer("easter-egg-speech", egg.text, egg.mode || "normal");
      }
    });
    growth.on("achievement", (data) => {
      sendToRenderer("growth-achievement", data);
    });
    growth.on("xp", (data) => {
      sendToRenderer("growth-xp", data);
    });
